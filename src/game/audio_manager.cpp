...
void AudioManager::_ready() {
    ...
    bgm_player->set_bus("Music"); // 绑定到系统的 Music 总线（可选，便于后续独立调音量）
...
void AudioManager::play_bgm(const StringName &bgm_name, bool loop) {
    ...
    // 4. 将通用 Node* 转换为目标的 VideoStreamPlayer*
    ...
    // 1. 如果新传入的歌正在播放，直接返回，防止重复触发导致音乐从头重播
    ...
    if (!bgm_registry.has(bgm_name)) {
        UtilityFunctions::push_error("BGM未注册: " + String(bgm_name));
        return;
    }
    ...
    if (bgm_player && !stream.is_null()) {
        ...
        // 注意：Godot 4 的循环属性通常是在导入音频资源（如 .mp3/.wav）时
        // 在编辑器的 "Import" 面板中勾选 Loop。代码里可以用 set_stream_paused 恢复。
