...
private:
  ...
  // ⚙️ 弹幕控制参数（放在开头，方便修改）
  // ==========================================
  double spawn_min_x = 72;     // 🌟 屏幕生成左边界
  double spawn_max_x = 936;    // 🌟 屏幕生成右边界
  int interval_frames = 1;        // 每隔多少帧发射（1表示每帧都发）

  double target_y_base = 234.0f;   // 目标 Y 轴中心点
  double target_y_range = 60.0f;   // 目标 Y 轴随机范围（234 ± 30）

  double stage1_base_deg = 90.0f;  // 基础方向朝下（90度）
  double stage1_swing_amp = 30.0f; // 摆动幅度（左右各 35 度）
  double stage1_swing_speed = 1;// 摆动速度/频率
