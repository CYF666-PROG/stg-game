...
// 1. 核心变速算法：根据当前帧小怪的速度累加位移，并计算距离进度
bool utility::Gearbox::change_speed(int hit_frame, game::enemy::Minion* minion, double initial_speed, double &acc_dist) {
...
void utility::Gearbox::_physics_process(double delta) {
    ...
    // 核心修复：在循环里动态擦除 multimap 节点时，必须通过接收 erase 返回的下一个有效迭代器来防止野指针崩溃
    for (auto it = hited_minion.begin(); it != hited_minion.end(); /* 后面手动递增 */) {
        ...
        if (!minion) {
            it = hited_minion.erase(it); // 安全删除，并返回下一任迭代器
            continue;
        }
        ...
        // 传入该小怪独立的结构体属性 `accumulated_distance` 的引用
...
void utility::Gearbox::_bind_methods() {
    ...
    // 向 Godot 注册属性，变量名改成 "target_distance" 以便在 Godot 编辑器 Inspector 面板中直观调整
